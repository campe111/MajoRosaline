...
export default function Products() {
  return (
    <section id="productos" className="bg-beige py-20">
      <div className="mx-auto w-full max-w-6xl px-6">
        <div className="text-center">
          <p className="text-sm uppercase tracking-[0.3em] text-olive/80">Productos</p>
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-3">
          {products.map((product, index) => (
            <motion.article
              key={product.name}
              className="group flex flex-col overflow-hidden rounded-3xl border border-warmGray/30 bg-white shadow-[0_40px_60px_-40px_rgba(47,47,47,0.35)]"
              initial="hidden"
              whileInView="visible"
            ...
            >
              <div className="relative h-56 overflow-hidden bg-gradient-to-br from-blush/70 via-beige to-white">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(107,142,35,0.25),transparent_60%)]" />
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_10%,rgba(232,200,200,0.45),transparent_55%)]" />
                <div className="absolute inset-6 rounded-2xl border border-white/60" aria-hidden />
                <p className="absolute bottom-6 left-6 text-lg font-medium uppercase tracking-[0.3em] text-softBlack/60">
                  Rosaline
                </p>
              </div>
              <div className="flex flex-1 flex-col gap-4 p-8">
                <h3 className="text-2xl">{product.name}</h3>
                <p className="text-sm leading-relaxed text-softBlack/70">{product.description}</p>
                <div className="mt-auto flex items-center justify-between text-sm text-softBlack/60">
